...
import {TouchableOpacity,ToastAndroid, FlatList, TouchableWithoutFeedback,ActivityIndicator , TouchableNativeFeedback , Modal , TextInput, Image,Text, View, StyleSheet, Alert, SafeAreaView, ScrollView, Picker, StatusBar, Button, Dimensions, Easing, Animated, AppState } from 'react-native';

//https://facebook.github.io/react-native/docs/signed-apk-android.

//TENGO LA NOCION DE QUE ESTO FUNCIONARA DE LA MISMA FORMA, EN LA QUE FUNCIONARIA VisualizarHoja como Visitante
...
import MostrarComentariosDeHoja from "./mostrarComentariosDeHoja.js";
/*
...
const VisualizarHojaDelApartadoPublico = (props) => {
	...
	*/
		...
		eliminacionDeComponentesEnElServidor: async () => {
			...
			if(datosEspecificosR.current.arregloDeComponentesAEliminarse.length > 0){
				...
				.catch((error) => {
					...
					ToastAndroid.show('Rayos hubo un error' , ToastAndroid.SHORT);
  					console.log("error linea 333: ",error)
				});
			}
			else{
				datosEspecificosR.current = {
				...
				<View style={{ width : '100%' , height : '50%' , position : 'absolute', top : 0 , }}>
					...
					<View style={{ width :  '100%' , height : '50%' , justifyContent : 'flex-start' , flexDirection : 'row'}}>
						...
						<Text style={{position : 'absolute' , right : '1%', bottom : 0}}>
							{(datosDelComponenteS.estado === 1) ? //xd
								'00:00:00'
							:
								<>
									{(datosDelComponenteS.cargandoAudio) ?
	...
	const ComponenteDeTexto = (props) => {
		...
		return(
			<View style={estilos.contenedorPrincipal}>
				<View style={estilos.contenedorSecundario}>
				<Text style={{fontFamily: "Viga-Regular",fontSize: 14,margin:3, color: "#111"}}>{datosDelComponenteS.contenidoInicial}</Text>
				</View>
			</View>
		);
		...
		:
			{
				...
				height: '100%',
				flexDirection: 'row',
				backgroundColor: '#c8d6e5'//aqui toy
			},
	});
  ...
  return(
                ...
                ListEmptyComponent={
                    ()=>(
						<View style={{width:"80%",padding:10,justifyContent:"center",alignSelf:"center",alignItems:"center",backgroundColor:"#111",borderRadius:10}}>
							<Text style={{textAlign:"center",color:"#fff",fontSize:16,fontFamily: "Viga-Regular"}}>Aun no Cuenta Con Componentes.</Text>
						</View>
                      )
                }
                renderItem={
	...
	const BotonDeMenu = () => {
		...
		const ejesX = useRef({
			menuDesplegableAgregar : new Animated.Value(((datosDelComponenteR.current.separadorEntreFuncionalidades / 2) * 3) + (datosDelComponenteR.current.anchoInicial * 2))
		})
			...
			botonAgregarComponenteDeArchivo: () => {
				...
				*/
				...
				FilePickerManager.showFilePicker(null, (response) => {
					...
					if(response.type === "video/mp4" || response.type === "video/mpeg" || response.type === "video/mpg" || response.type === "video/mov" || response.type === "video/wmv" || response.type === "video/avi" || response.type === "video/mkv"
						|| response.type === "audio/mp3" || response.type === "audio/m4a" || response.type === "audio/mpeg" || response.type === "audio/mpg"
						|| response.type === "image/jpeg" || response.type === "image/jpg" || response.type === "image/png" || response.type === "image/gif" || response.type === "image/bmp" || response.type === "image/svg"
					){
						...
						if(response.size <= 150000000){
							...
							else if(response.type === "audio/mp3" || response.type === "audio/m4a" || response.type === "audio/mpeg" || response.type === "audio/mpg"){
								//ComponenteDeAudio
								tipoDeComponente = 'AUDIO';
							}
							else if(response.type === "image/jpeg" || response.type === "image/jpg" || response.type === "image/png" || response.type === "image/gif" || response.type === "image/bmp" || response.type === "image/svg" ){
			...
			botonAgregarComponenteDeTexto: async () => {
				...
				.then((respuesta) => {
					if(respuesta === "Exito"){
						...
						ToastAndroid.show('Rayos hubo un error' , ToastAndroid.SHORT);
  						console.log("error linea : 4326")
					}
				})
				.catch((error) => {
					ToastAndroid.show('Rayos hubo un error' , ToastAndroid.SHORT);
  				    console.log("error linea : 4331")
				});
			}
		...
		*/
		const [modalComentariosHojaVisible,setModalComentariosHojaVisible] = useState(false)
		...
		<>
					...
					<View style={estilos.contenedorDelMenuDesplegablePrincipal}>
						<Animated.View style={[estilos.menuDesplegablePrincipal]}>
							<TouchableNativeFeedback onPress={()=>{
								...
								setModalComentariosHojaVisible(true)
							}}>
								<View style={[estilos.botonFuncionalidad, {top: datosDelComponenteR.current.finalizadorArribaYAbajo}]}>
									<Icon10 name={"comments"} size={datosDelComponenteR.current.tamanioDeFuncionalidad * .7} color={'black'}/>
								</View>
							</TouchableNativeFeedback>
						</Animated.View>
					</View>
			...
			{(modalComentariosHojaVisible) ?
				<MostrarComentariosDeHoja eventoSalir={()=>setModalComentariosHojaVisible(false)} idHoja={""+variablesDentroDeMochila.eleccionIdDeLibreta + "_" + variablesDentroDeMochila.eleccionIdDeHoja + "_" + variablesDentroDeMochila.matriculaDelPropietario} /> : null
			 }
		</>
		);
	}
...
export default VisualizarHojaDelApartadoPublico;
//=) xd :v :> ... ...