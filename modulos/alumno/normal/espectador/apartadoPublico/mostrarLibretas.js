...
import {TouchableOpacity,ToastAndroid, TouchableNativeFeedback, TextInput, Image,Text, View, StyleSheet, Alert, SafeAreaView, ScrollView, Picker, StatusBar, Button, Dimensions, ActivityIndicator, Modal } from 'react-native';
												...
												:
													<>
														{(datosDelComponenteS.respuestaDelServidorPorCreacionDeCopia === null) ?
															<>
																<Text>{"Si no hay alguna copia de esta libreta en el apartado privado, esta se copiará; sin embargo, si ya la hay, no se hará nada. ¿Estás seguro de que quieres Crear Copia En Apartado Privado?"}</Text>
		...
		let promesa = await fetch('http://backpack.sytes.net/servidorApp/php/accionesEnApartados/mostrar/traerDatosDeHojasAMostrarLibretasDelApartadoElegido.php',{
			...
			body:datos,
							...
							.catch((error) => {
								...
								conole.log("514 = linea error: ",error)
							});
