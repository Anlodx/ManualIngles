...
const MostrarHojasDeLibretaDelApartadoPublico = (props) => {
								...
								<View style={{width: "100%", height: "90%", overflow: "hidden"}}>
									<ScrollView contentContainerStyle={{ alignItems: "center" }}>
												...
												<View style={{width: "90%", height: (datosDelComponenteR.current.alturaDeScrollView) * .15, borderRadius: 20, backgroundColor: "lavender", borderWidth: 2, overflow: "hidden"}}>
													<TouchableNativeFeedback onPress={() => {
														...
														<View style={{width: "100%", height: "100%", borderRadius: 20, justifyContent: "center"}}>
															<Text style={{position: "absolute", left: "10%"}}>{"Crear Copia En Apartado Privado"}</Text>
												...
												:
													<>
														{(datosDelComponenteS.respuestaDelServidorPorCreacionDeCopia === null) ?
															<>
																<Text>{"Si no hay alguna copia de esta hoja en el apartado privado, esta se subirá; sin embargo, si ya la hay, no se hará nada. ¿Estás seguro de que quieres Crear Copia En Apartado Privado?"}</Text>
										...
										: (datosDelComponenteS.estadoDelModal === 3) ?
											<>
												<Text style={{alignSelf: 'center'}}>{"¿Estás seguro de que quieres eliminar esta hoja?"}</Text>
		...
		if((variablesDentroDeMochila.arreglosDeHojas.idsDeElemento).length === 0 && (variablesDentroDeMochila.arreglosDeHojas.nombres).length === 0 && (variablesDentroDeMochila.arreglosDeHojas.numerosDeComponentes).length === 0){
			...
			*/
			...
			for(let i = 0; i < cantidad;i++){
				...
				//elemento = 'hoja1'
				arreglo.push(<BotonDeHoja key={"" + i} elemento={variablesDentroDeMochila.arreglosDeHojas.idsDeElemento[i]} nombreDentroDeCaja={variablesDentroDeMochila.arreglosDeHojas.nombres[i]} edicion={bool} cantidadDeComponentesDentro={variablesDentroDeMochila.arreglosDeHojas.numerosDeComponentes[i]}/>);

				if((i + 1) === cantidad){
					//ES LA ULTIMA POSICION
					//ES EL ULTIMO BOTON
					arreglo.push(<View key={"ViewFinal"} style={{height:100}} />);
				}
				else{
					//NO ES LA ULTIMA POSICION
					//NO ES EL ULTIMO BOTON
					arreglo.push(<View key={"View" + i} style={{height:22}} />);
				}
			}
		}
		return (arreglo);
	...
	useEffect(() => {
		console.log("Informacion de mochila visitante => ", variablesDentroDeMochila);
	}, []);
