...
const Main = () => {
    ...
    useEffect(()=>{
	console.log("useEffect acabo xd quiz")
  ...
  <Text style={{width:WIDTH,textAlign:"center",fontSize:16, color: "#ff6b6b", fontWeight:"bold" }}>Puntaje: <Text style={{fontWeight:"bold",color:"#222f3e"}}> {rating} </Text> </Text>
		...
		ListEmptyComponent={()=>(
			<View style={{width:WIDTH * 0.8 , padding: 15, backgroundColor: "#eccc68",alignSelf:"center", borderRadius:10,marginTop:10}}>
				<Text style={{textAlign:"center",fontWeight:"bold"}}>Espere un momento...</Text>
			</View>
		)}
	    	renderItem={({item})=>{
...
const ComponentePregunta = (props) =>{
            ...
            <Text style={{marginBottom:5,textAlign:"center",fontWeight:"bold", color:"#30336b"}}>¿Qué es <Text style={{fontWeight:"bold",color:"#1abc9c"}}>"{objeto.verb}"</Text>?</Text>
	{
		...
		<>
			...
			setPulsado(1)
			setResultado(true)
		}} style={{padding:6,backgroundColor:"#feca57", padding: 15, justifyContent:"center", borderRadius:5 }}>
                    <Text style={{textAlign:"center" ,fontWeight:"bold"}}>{objeto.meaning}</Text>
                </TouchableOpacity>
		...
		<TouchableOpacity onPress={()=>{
			...
			setResultado(false)
                    ...
                    <Text style={{textAlign:"center" ,fontWeight:"bold"}}>{distractor}</Text>
                </TouchableOpacity>
		</>
		) : (
		<>
